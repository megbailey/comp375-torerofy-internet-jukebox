...
void event_loop(int epoll_fd, int server_socket, char *dir, std::vector<fs::path> pathToMP3, std::vector<fs::path> pathToInfo);
...
int main(int argc, char **argv) {
	 ...
	 */
    cout << "Path to songs: " << mp3Dir << "\n";

	vector<fs::path> pathToMP3 = find_mp3_or_info_files(mp3Dir, false);
	vector<fs::path> pathToInfo = find_mp3_or_info_files(mp3Dir, true);
	...
	event_loop(epoll_fd, serv_sock, mp3Dir, pathToMP3, pathToInfo);
}
 ...
 * 				to check.
 *
 * @return a path to mp3
 */
std::vector<fs::path> find_mp3_or_info_files(char *dir, bool info) {
    uint32_t num_mp3_files = 0;
    uint32_t num_info_files = 0;
    vector<fs::path> pathToMP3 (1);
    vector<fs::path> pathToInfo (1);

    for (fs::directory_iterator entry(dir); entry != fs::directory_iterator(); ++entry) {
        string filename = entry->path().filename().string();
        if (entry->path().extension() == ".mp3") {
            if (num_mp3_files == pathToMP3.size()) {
                pathToMP3.resize(pathToMP3.size() * 2);
            }
            pathToMP3[num_mp3_files] = entry->path();
            cout << "added: " << filename << "to the vector\n";
            num_mp3_files++;
            ...
            if (fs::is_regular_file(info_file_path)) {
                if (num_info_files == pathToInfo.size()) {
                    pathToInfo.resize(pathToInfo.size() * 2);
                }
                pathToInfo[num_info_files] = info_file_path;
                num_info_files++;
 ...
 */
//added dir to parameters so that we can call get list
void event_loop(int epoll_fd, int server_socket, char *dir, vector<fs::path> pathToMP3, vector<fs::path> pathToInfo) {
		...
		for (int n = 0; n < num_events; n++) {
			...
			// this socket)
				...
				if (events[n].data.fd == server_socket) {
					 ...
					 */
					clients[events[n].data.fd].handle_input(epoll_fd, dir, pathToMP3, pathToInfo);
				}
            }
			...
			// how you are handling clients.
				 ...
				 * you had in progress.
				 */
				// Create a new function in your ConnectedClient class
